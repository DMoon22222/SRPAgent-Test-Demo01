...
`pilot` 是一个面向代码仓库的轻量本地 coding agent。它直接跑在终端里，先看当前工作区，再用一组受约束的工具去读文件、改文件、跑命令，并把会话状态保存在本地 `.pico/` 目录里。
...
```bash
uv run pilot --cwd /path/to/repo
```

直接跑一次性任务：

```bash
uv run pilot "inspect the test failures and propose a fix"
```
...
```bash
uv run pilot --model deepseek-v4-pro --base-url https://api.deepseek.com/anthropic
```
...
```bash
uv run pilot --provider openai
uv run pilot --provider anthropic
```
...
uv run pilot --provider ollama --model qwen3.5:4b
```
